...
directory = 'downloaded-content-focused-search-bfs'
max_depth = 5
queue_href = []
...
def crawl(seed_page, count):
    ...
    for tags in hrefs:
        ...
        if valid(domain_name, tags['href'], anchor_text) and len(queue_href) < 1500 and len(queue) < 1500:
            queue.append(link)
...
def print_links_in_crawled(crawled):
    for i in crawled:
        with open('crawled-task2-bfs.txt', 'a') as f:
            f.write(i + '\n')
